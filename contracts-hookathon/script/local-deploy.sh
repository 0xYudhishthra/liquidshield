...
case "$CHAIN" in
    base)
        ...
        ;;
    base-sepolia)
        FORK_RPC="${BASE_SEPOLIA_RPC_URL:-https://sepolia.base.org}"
        ;;
    unichain)
        FORK_RPC="${UNICHAIN_RPC_URL:-https://mainnet.unichain.org}"
        ;;
    unichain-sepolia)
        FORK_RPC="${UNICHAIN_SEPOLIA_RPC_URL:-https://sepolia.unichain.org}"
        ;;
    *)
        echo "ERROR: Unsupported chain '$CHAIN'. Use 'base', 'base-sepolia', 'unichain', or 'unichain-sepolia'."
        exit 1
        ;;
esac
...
anvil --fork-url "$FORK_RPC" --port "$ANVIL_PORT" --chain-id 696969 --silent &
ANVIL_PID=$!
